...
void Cameras::RefreshAvailableCameras() {
    ...
    // Summary
    switch (List.count()) {
        case 0:     qInfo() << "No camera detected"; break;
        case 1:     qInfo() << "1 camera detected"; break;
        default:    qInfo().nospace() << List.count() << " cameras detected"; break;
    }
...
void Cameras::displayCamerasInfos() {
    ...
    for (int i=0; i<List.count(); i++) {
        if (List[i].type == "FLIR") { List_FLIR[List[i].index]->display_info(); }
    }
