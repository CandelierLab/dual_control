...
Camera_FLIR::Camera_FLIR(CameraPtr cPtr, int identifier) : pCam(cPtr), id(identifier) {
  // --- Initialization
  grabState = false;
  ...
  if (IsAvailable(category) && IsReadable(category)) {
    ...
    QRegExp rx(" (\\w+)\-");
    QString model("");
    if (rx.indexIn(QString(features.at(3)->ToString())) != -1) {
      model += rx.cap(1) + " ";
    }
...
void Camera_FLIR::display_info() {
  ...
  if (IsAvailable(category) && IsReadable(category)) {
    ...
    qWarning() << "Device info not available";
  }
}
  ...
  while (grabState) {
    ...
    if (pImg->IsIncomplete()) {
      ...
      for (unsigned int i = 0; i <= 255; i++) {
        SImg.Img.setColor(i, qRgb(i, i, i));
      }
