...
Dual::Dual(QVector<QRect> screens, QString path, int id, QWidget *parent) : Screen(screens), path(path), guiid(id), QMainWindow(parent), ui(new Ui::Dual) {
...
void Dual::autoset() {
  // Create data folder?
  if (!QDir(ui->DataPath->text()).exists()) {
    QDir().mkdir(ui->DataPath->text());
  }
  ...
  foreach (const QFileInfo &elm, FIL) {
    if (QDir(elm.absoluteFilePath()).exists()) {
      ...
      if (tmp.count() == 2 || !tmp.at(0).compare("Run")) {
        tmp = tmp.at(1).split(".");
        if (tmp.at(0).toInt() == guiid && tmp.at(1).toInt() > nRun) {
          nRun = tmp.at(1).toInt();
        }
      }
    }
...
void Dual::GrabLoop() {
  ...
  if (ImgFile.open(QIODevice::WriteOnly | QIODevice::Append)) {
    ...
    tmp << endl
        << "#" + QString("Timestamp:%1").arg(TimeStamp);
...
void Dual::snapshot() {
  ...
  foreach (QString file, flist) {
    ...
    tmp = tmp.at(1).split(".");
    if (tmp.at(0).toInt() > nSnap) {
      nSnap = tmp.at(0).toInt();
    }
  }
  nSnap++;
  qDebug() << nSnap;
...
void Dual::ProtocolLoop() {
  ...
  int hours = floor(m / 3600000);
  m -= hours * 3600000;
  int minutes = floor(m / 60000);
  m -= minutes * 60000;
...
void Dual::setValve(int nV, bool b) {
  ...
  switch (nV) {
    case 1:
      pV = ui->pinValve1->value();
      break;
    case 2:
      pV = ui->pinValve2->value();
      break;
    case 3:
      pV = ui->pinValve3->value();
      break;
    case 4:
      pV = ui->pinValve4->value();
      break;
    case 5:
      pV = ui->pinValve5->value();
      break;
    case 6:
      pV = ui->pinValve6->value();
      break;
  }
...
void Dual::setCircuitLeft(int i) {
  switch (i) {
    case 0:  // Injection
      ...
      if (ui->CircuitRight->currentIndex() > 0) {
        ui->CircuitRight->setCurrentIndex(0);
      }
      break;
    ...
    case 1:  // Buffer
      ...
      if (ui->CircuitRight->currentIndex() == 0) {
        ui->CircuitRight->setCurrentIndex(1);
      }
      break;
...
void Dual::setCircuitRight(int i) {
  switch (i) {
    case 0:  // Injection
      ...
      if (ui->CircuitLeft->currentIndex() > 0) {
        ui->CircuitLeft->setCurrentIndex(0);
      }
      break;
    ...
    case 1:  // Buffer
      ...
      if (ui->CircuitLeft->currentIndex() == 0) {
        ui->CircuitLeft->setCurrentIndex(1);
      }
      break;
